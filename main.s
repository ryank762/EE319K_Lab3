;****************** main.s ***************
; Program written by: Ryan Kim (jk39938)
...
;	Works likes a flip-flop
	...
	ADDS		R0, #0				;set flag=input
	BEQ		WAIT_1					
	BIC		R5, #0x01				;toggled to "ready to read" once PE1 is pressed
	B		WAIT_1
;
READY_TO_READ
	LDR		R1, =GPIO_PORTE_DATA_R
	...
	LSR		R0, #1					
	ADDS		R0, #0				;set flag=input
	BNE		WAIT_1				;loops if PE1 is still pressed
	ORR		R5, #0x01				
	B		loop				;change duty cycle if PE1 is unpressed
;
BREATHING
...
AGAIN_4
	...
	LSR		R0, #4
	ADDS		R0, #0				
	BNE		RETURN_TO_MAIN			;check if PF4 during execution of breathing loop
;
	MOV		R1, #1
	BL		LED_DISPLAY
	MOV		R1, R6
